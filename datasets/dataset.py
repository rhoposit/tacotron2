...
class TargetData(
    namedtuple("TargetData",
               ["id", "codes", "codes_width", "target_length", "done",
                "codes_loss_mask", "binary_loss_mask"])):
    pass
...
class DatasetSource:
    ...
    def _prepare_target(self):
        def convert(target: PreprocessedTargetData):
            ...
            codes = tf.cond(no_padding_condition, lambda: target.codes, padding_function(target.codes))
            codes.set_shape((None, self.hparams.num_freq))
            ...
            # loss mask
            codes_loss_mask = tf.ones(shape=padded_target_length, dtype=tf.float32)
            binary_loss_mask = tf.ones(shape=padded_target_length // r, dtype=tf.float32)

            return TargetData(target.id, codes, target.codes_width, padded_target_length, done, codes_loss_mask, binary_loss_mask)
    ...
    def _decode_target(self):
        return self.target.map(lambda d: decode_preprocessed_target_data2(parse_preprocessed_target_data2(d)))
    ...
    def group_by_batch(self, batch_size=None):
        ...
        def reduce_func(unused_key, window: tf.data.Dataset):
            return window.padded_batch(batch_size, padded_shapes=(
                ...
                TargetData(
                    id=tf.TensorShape([]),
                    codes=tf.TensorShape([None, self.hparams.num_freq]),
                    codes_width=tf.TensorShape([]),
                    target_length=tf.TensorShape([]),
                    done=tf.TensorShape([None]),
                    codes_loss_mask=tf.TensorShape([None]),
                ...
                TargetData(
                    id=tf.to_int64(0),
                    codes=tf.to_float(0),
                    codes_width=tf.to_int64(0),
                    target_length=tf.to_int64(0),
                    done=tf.to_float(1),
                    codes_loss_mask=tf.to_float(0),
